...
//믽��馭򻠢󷨲ᮨ��321550.00   -1355063.70      14957.00
...
*/
    ...
    for (k = 0; k < modelArr.length; k++) {
        if (modelArr[k] == "CNTB" && modelArr[k + 5] != "PRIM" && modelArr[k + 5] != "CNTE") {
            //AddLi(modelArr[k + 2])
            AddUl(modelArr[k + 2]);
        }
            ...
            if (mesh != null) {
                ...
                if (group3 != null)
                    mesh.name = group3.name + "_" + meshCount;
                else
                    mesh.name = meshCount;
                //}
                //AddLi(mesh.name);
...
function createMeshFromPrim(textArray, material, grPosArr) {
        ...
        case "    11":
            ...
            for (var i = 7; i < textArray.length - 1; i++) {
                ...
                if (textList.length == 4) {
                    ...
                    //if (polyCursor) {
                    //    if (!isNaN(parseFloat(textList[1])) && !isNaN(parseFloat(textList[2])) && !isNaN(parseFloat(textList[3]))) {
                    //        var vertX = parseFloat(textList[1]);
                    //        var vertY = parseFloat(textList[2]);
                    //        var vertZ = parseFloat(textList[3]);

                    //        if (currentX != null && currentX != vertX) {
                    //            commonX = false;
                    //        }
                    //        if (currentY != null && currentY != vertY) {
                    //            commonY = false;
                    //        }
                    //        if (currentZ != null && currentZ != vertZ) {
                    //            commonZ = false;
                    //        }
                    ...
                    if (vertexCount > 0) {
                       var vertices = [];
                        ...
                        //console.log('commonY: ' + commonY);
                        //console.log('commonZ: ' + commonZ);

                        var firstVertex = geometry.vertices.length - vertexCount;
                        for (var t = firstVertex ; t < geometry.vertices.length; t++) {
                            //geometry.faces.push(new THREE.Face3(firstVertex, t-1, t + 1));
                            var vertex = [geometry.vertices[t].x, geometry.vertices[t].y, geometry.vertices[t].z];
                            vertices.push(vertex);
                            /*
                            var vert = new Object();
                            console.log(geometry.vertices[t]);
                            if (commonX) {
                                vert.x = geometry.vertices[t].y;
                                vert.y = geometry.vertices[t].z;
                                vertices.push(vert);
                            }
                            else if (commonY) {
                                vert.x = geometry.vertices[t].x;
                                vert.y = geometry.vertices[t].z;
                                vertices.push(vert);
                            }
                            else if (commonZ) {
                                vert.x = geometry.vertices[t].x;
                                vert.y = geometry.vertices[t].y;
                                vertices.push(vert);
                            }
                            */
                        }
                        ...
                        if (vertices.length >= 3) {
                            ...
                            //        triangList.map(function (tri) { return tri} ).join() :
                            //        'NO Triangle-List!';
                            //console.log(resultStr);
                            //var triangles = resultStr.split(',');
                ...
                /*
				if (polyCursor) {
					geometry.vertices.push(parseFloat(textList[1]), parseFloat(textList[2]), parseFloat(textList[3]));
				}
				else {
					geometry.faces.push( new THREE.Face3( parseFloat(textList[1]), parseFloat(textList[2]), parseFloat(textList[3]) ) );
				}
				polyCursor = !polyCursor;
				}
				*/
            ...
            /*
            var f = 0;
            for (var i = 0; i < polyPosArr.length - 12; i += 12, f += 4) {

                console.log(i);

                var v1 = new THREE.Vector3(polyPosArr[i], polyPosArr[i + 1], polyPosArr[i + 2]);
                var v2 = new THREE.Vector3(polyPosArr[i+3], polyPosArr[i + 4], polyPosArr[i + 5]);
                var v3 = new THREE.Vector3(polyPosArr[i + 6], polyPosArr[i + 7], polyPosArr[i + 8]);
                var v4 = new THREE.Vector3(polyPosArr[i + 9], polyPosArr[i + 10], polyPosArr[i + 11]);
                ...
                geometry.faces.push(new THREE.Face3(f, f + 2, f + 3));
                geometry.faces.push(new THREE.Face3(f + 3, f + 1, f + 2));
            ...
            */
            ...
            //geometry.addAttribute('position', new THREE.BufferAttribute(polyPosArr, 3));
            //geometry.addAttribute('normal', new THREE.BufferAttribute(polyNormArr, 3));
            //geometry.addAttribute('color', new THREE.BufferAttribute(polyColorArr, 3));
            //geometry.addAttribute('uv', new THREE.BufferAttribute(polyUvArr, 2));
            ...
            //var returnMesh = new THREE.Mesh(mesh.geometry, selectedMaterial);
    ...
    //    var tmpGeommm = new THREE.BufferGeometry().fromGeometry(mesh.geometry);

    //    modelGeometry = new THREE.BufferGeometry();/*.fromGeometry( geometry );*/

    //    for (var key in tmpGeommm.attributes) {

    //        modelGeometry.addAttribute(key, tmpGeommm.getAttribute(key));
    //        //	console.log(tmpGeommm.attributes[key]);
    //    }

    //    //console.log(tmpGeommm);
    ...
    return returnMesh;
    //			return geometry;
    //var bufferGeometry = new THREE.BufferGeometry().fromGeometry( geometry, settings );
    ...
    if (modelGeometry == null)
    {
        var qgeometry = new THREE.SphereGeometry( 1 , 1, 1 );
        qgeometry.applyMatrix ( matx );

        modelGeometry = new THREE.BufferGeometry().fromGeometry(qgeometry);
...
console.log(text)    
    ...
    ul.className += "dropdown btn-default";
    li.appendChild(ul);
    modelExplorer = ul;
}